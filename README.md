...
**Idle agents can be pushed, not just waited on.** Sending shows a desktop
notification, and `bridge send --to reasonix --wake` (or `bridge wake codex`)
runs the agent headless to handle it right now.
...
| `bridge send --to <agent> --subject "..." [--body "..."] [--wake]` | Hand off a task |
