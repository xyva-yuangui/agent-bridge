...
pub fn post(request: &Request) -> Result<String, String> {
    ...
    let Request::Post {
        ...
        return Err("post request expected".to_owned());
    };
    ...
    toast
        .SetTag(&HSTRING::from(&notification_id[6..22]))
        .map_err(|error| error.to_string())?;
    toast
        .SetGroup(&HSTRING::from("agent-bridge"))
        .map_err(|error| error.to_string())?;
    let unix_ticks = SystemTime::now()
        ...
        * 10_000_000;
    let expiry = DateTime {
        UniversalTime: 116_444_736_000_000_000i64
            + unix_ticks
            + i64::from(*expires_in_seconds) * 10_000_000,
    };
    let expiry_ref: IReference<DateTime> = PropertyValue::CreateDateTime(expiry)
        .map_err(|error| error.to_string())?
        .cast()
        .map_err(|error| error.to_string())?;
    toast
        .SetExpirationTime(&expiry_ref)
        .map_err(|error| error.to_string())?;
...
#[cfg(not(windows))]
pub fn post(_: &Request) -> Result<String, String> {
    Err("Windows WinRT toast APIs are unavailable on this platform".to_owned())
}
...
mod tests {
    ...
    fn notification_identifier_is_stable_and_opaque() {
        assert_eq!(
            stable_notification_id("task-1"),
            stable_notification_id("task-1")
        );
        assert_ne!(
            stable_notification_id("task-1"),
            stable_notification_id("task-2")
        );
