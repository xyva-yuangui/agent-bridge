...
pub fn post(request: &Request) -> Result<String, String> {
    ...
    let action_xml = actions.iter().map(|action| {
        ...
        format!("<action content=\"{}\" arguments=\"{}://action/{}?notification_id={}&amp;task_id={}\" activationType=\"protocol\"/>", label, PROTOCOL, name, xml_escape(&notification_id), xml_escape(task_id))
    }).collect::<String>();
    // XmlDocument is the WinRT input format. Every untrusted text/attribute is escaped before insertion.
    let xml = format!("<toast launch=\"{}://action/view?notification_id={}&amp;task_id={}\"><visual><binding template=\"ToastGeneric\"><text>{}</text><text>{}</text></binding></visual><actions>{}</actions></toast>", PROTOCOL, xml_escape(&notification_id), xml_escape(task_id), xml_escape(title), xml_escape(body), action_xml);
