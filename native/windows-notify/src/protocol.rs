...
impl Response {
    ...
    pub fn registered(detail: impl Into<String>) -> Self {
        Self {
            ok: true,
            notification_id: "registration".to_owned(),
            status: "os_posted".to_owned(),
            detail: bounded_detail(detail.into()),
        }
    }
...
pub fn parse_request(input: &[u8]) -> Result<Request, String> {
    if input.len() > MAX_INPUT_BYTES {
        return Err("request exceeds 16384 bytes".to_owned());
    }
    let value: serde_json::Value =
        serde_json::from_slice(input).map_err(|_| "malformed request JSON".to_owned())?;
    enforce_exact_fields(&value)?;
    let request: Request =
        serde_json::from_value(value).map_err(|_| "malformed request JSON".to_owned())?;
    validate(&request)?;
    Ok(request)
}
...
fn enforce_exact_fields(value: &serde_json::Value) -> Result<(), String> {
    ...
    let operation = object
        ...
        .ok_or_else(|| "request operation is missing".to_owned())?;
...
fn validate(request: &Request) -> Result<(), String> {
    match request {
        Request::Post {
            ...
            if actions.as_slice() != [Action::View, Action::Claim, Action::Snooze] {
                return Err("actions must be view, claim, snooze".to_owned());
            }
            if *expires_in_seconds == 0 || *expires_in_seconds > 86_400 {
                return Err("invalid expires_in_seconds".to_owned());
            }
        }
        Request::Action {
            action: _,
        ...
        Request::Register { activation_argv } => {
            if activation_argv.is_empty()
                || activation_argv.len() > 16
                || !activation_argv
                    .iter()
                    .all(|item| !item.is_empty() && item.len() <= 1024)
            {
                return Err("invalid activation_argv".to_owned());
            }
        }
