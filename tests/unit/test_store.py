...
class StoreTests(unittest.TestCase):
    ...
    def test_open_applies_pragmas_and_initial_schema(self):
        ...
        self.assertEqual(store.scalar("SELECT MAX(version) FROM schema_migrations"), 4)
    ...
    def test_non_initial_migration_creates_readable_restorable_backup(self):
        ...
        store.close()
        third_migration = (5, "CREATE TABLE migration_backup_probe (id INTEGER PRIMARY KEY);")
        ...
        backups = list(self.root.glob("agent-bridge.sqlite3.before-v5.*.bak"))
    ...
    def test_v1_database_upgrades_to_v2_launch_reservations(self):
        ...
        try:
            self.assertEqual(upgraded.scalar("SELECT MAX(version) FROM schema_migrations"), 4)
